window.app.views.Modal = Backbone.View.extend({
	...
	render: function() {
		...
		this.$('#modal-window').on('shown.bs.modal', function() {
			if (self.$innerEl) {
				self.$('#modal-body').append(self.$innerEl);
			}
			if (self.onIni) {
				self.onIni();
			}
		...
		if (this.bodyClass) {
			this.$('.modal-body').addClass(this.bodyClass);
		}

		if (this.error) {
			this.$('.modal-header').addClass('error');
		}
		if (this.success) {
			this.$('.modal-header').addClass('success');
		}

		this.$('#modal-window').on('hidden.bs.modal', function() {
			self.$el.detach();
		});
